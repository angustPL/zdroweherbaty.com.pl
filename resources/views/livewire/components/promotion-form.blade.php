...
mount(function ($promotionId = null) {
    ...
    // Pobierz dostępne grupy (spłaszczona struktura)
    $hierarchicalGroups = Group::getHierarchicalStructure();
    $this->available_groups = $this->flattenGroups($hierarchicalGroups);
...
$loadPromotionData = function () {
    ...
    if ($this->promotionId) {
        $promotion = Promotion::with(['promotionProducts', 'promotionGroups'])->findOrFail($this->promotionId);
...
<div>
    ...
    @if ($showSuccessMessage)
        <div class="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg" x-data x-init="setTimeout(() => $wire.set('showSuccessMessage', false), 2000)">
            <div class="flex items-center">
                ...
                <p class="text-sm font-medium text-green-800">
                    {{ $promotionId ? 'Promocja została zaktualizowana.' : 'Promocja została utworzona.' }}
                </p>
            </div>
        </div>
    @endif

    <form wire:submit.prevent="save" id="promotion-form">
        <div class="space-y-6">
                ...
                <div class="p-4 pt-3 space-y-3">
                    ...
                    @if ($discount_type !== 'free_delivery')
                        <flux:field>
                            ...
                            <flux:input type="number" wire:model="discount_value" step="0.01" min="0"
                                required />
                        </flux:field>
                    @endif
                </div>
                            ...
                            <div x-data="{
                                ...
                                <div class="relative">
                                    <flux:input type="text" x-model="search" @keydown="handleKeydown"
                                        @focus="showSuggestions = search.length >= 2"
                                        @click.away="showSuggestions = false" placeholder="Wpisz nazwę grupy..." />
