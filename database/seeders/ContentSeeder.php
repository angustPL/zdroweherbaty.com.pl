...
class ContentSeeder extends Seeder
{
    ...
    {
        $this->seoPath = dirname(base_path()) . DIRECTORY_SEPARATOR . 'zdroweherbaty.com.pl - serwer' . DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR . 'views' . DIRECTORY_SEPARATOR . 'scripts' . DIRECTORY_SEPARATOR . 'seo';
    }
    ...
    /**
     * Seed regulaminu - treść bezpośrednio z blade template.
     */
    private function seedTerms(): void
    {
        $termsContent = '<h2 class="text-2xl font-bold text-gray-900 mb-6">Wstęp</h2>
                <ol>
                    <li>
        ...
        Content::updateOrCreate(
            ...
            [
                ...
                'is_active' => true,
                'meta' => [
                    'meta_title' => 'Regulamin - Zdrowe Herbaty BIFIX',
                    'meta_description' => 'Regulamin sklepu internetowego Zdrowe Herbaty BIFIX. Zasady sprzedaży, dostawy, płatności i reklamacji herbat BIFIX.',
                ],
            ]
        );
    ...
    /**
     * Seed treści dla wszystkich grup produktów z plików .phtml.
     */
    private function seedProductGroups(): void
    {
        $grupaPath = $this->seoPath . DIRECTORY_SEPARATOR . 'grupa';

        if (!File::isDirectory($grupaPath)) {
            $this->command->warn('Katalog grup produktów nie istnieje: ' . $grupaPath);
            return;
        }

        $files = File::glob($grupaPath . DIRECTORY_SEPARATOR . '*.phtml');
        $count = 0;
        $errors = 0;
        ...
        foreach ($files as $filePath) {
            try {
                ...
                $parsed = $this->parsePhtmlFile($filePath);

                if (empty($parsed['content'])) {
                    $this->command->warn('  ⚠ Pominięto: ' . $identifier . ' (brak treści)');
                    $errors++;
                    continue;
                }
                ...
                $this->command->error('  ✗ Błąd przy parsowaniu: ' . basename($filePath) . ' - ' . $e->getMessage());
                $errors++;
            }
        }
    ...
    /**
     * Seed treści dla strony głównej z pliku home.phtml.
     */
    private function seedHome(): void
    {
        $homePath = $this->seoPath . DIRECTORY_SEPARATOR . 'home.phtml';

        if (!File::exists($homePath)) {
            $this->command->warn('Plik home.phtml nie istnieje: ' . $homePath);
            return;
        }

        $parsed = $this->parsePhtmlFile($homePath, 'tresc');

        if (empty($parsed['content'])) {
            $this->command->warn('Nie można wyciągnąć treści ze strony głównej.');
            return;
        }
        ...
        Content::updateOrCreate(
            ...
            [
                'title' => $parsed['title'] ?? 'Strona główna',
                'content' => $parsed['content'],
                'is_active' => true,
                'meta' => [
                    'meta_title' => $parsed['meta_title'] ?? null,
    ...
    /**
     * Parsuj plik .phtml i wyciągnij z niego dane.
     *
     * @param string $filePath Ścieżka do pliku
     * @param string $placeholderName Nazwa placeholder (domyślnie 'grupaOpis', dla home użyj 'tresc')
     * @return array
     */
    private function parsePhtmlFile(string $filePath, string $placeholderName = 'grupaOpis'): array
    {
        ...
        // Meta Description
        if (preg_match('/headMeta\(\)\s*->\s*setName\s*\(\s*[\'"]description[\'"]\s*,\s*[\'"]([^\'"]+)[\'"]\s*\)/s', $content, $matches)) {
            $result['meta_description'] = $matches[1];
        } elseif (preg_match('/headMeta\(\)\s*->\s*setName\s*\(\s*[\'"]description[\'"]\s*,\s*substr\s*\(\s*[\'"]([^\'"]+)[\'"]/s', $content, $matches)) {
            $result['meta_description'] = $matches[1];
        }

        // Meta Title
        if (preg_match('/headTitle\s*\(\s*[\'"]([^\'"]+)[\'"]\s*\)/s', $content, $matches)) {
            $result['meta_title'] = $matches[1];
        }

        // H1/Title
        if (preg_match('/placeholder\s*\(\s*[\'"]title[\'"]\s*\)\s*->\s*set\s*\(\s*[\'"]([^\'"]+)[\'"]\s*\)/s', $content, $matches)) {
            $result['h1'] = $matches[1];
            $result['title'] = $matches[1];
        }

        // Content z placeholder
        $pattern = '/placeholder\s*\(\s*[\'"]' . preg_quote($placeholderName, '/') . '[\'"]\s*\)\s*->\s*captureStart\s*\(\s*\)\s*\?>\s*(.*?)\s*<\?php\s+\$this\s*->\s*placeholder\s*\(\s*[\'"]' . preg_quote($placeholderName, '/') . '[\'"]\s*\)\s*->\s*captureEnd\s*\(\s*\)/s';
